...
# Read in data
gdp_cat_df <- read.csv('./part2/data/cat_data.csv')
gdp_mono_df <- read.csv('./part2/data/mono_data.csv')

# Keep only numeric cols
num_cat <- gdp_cat_df[, c('length', 'year', 'gdp', 'hdi')]
num_mono <- gdp_mono_df[, c('length', 'year', 'gdp', 'hdi')]
